...
if (navigator.geolocation) {
  // 2 arguments avec leurs propres callback
...
async function getWeatherData(long, lat) {
  try {
    const results = await fetch(
      `https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${long}&exclude=minutely&units=metric&lang=fr&appid=a783ff1b655a8efef32760dc98437171`
    );
...
// Affichage des heures et leurs temperatures
function handleHours(data) {

  hourNameBlocks.forEach((block, index) => {
    // Permet d'avoir 1h incrémenter
